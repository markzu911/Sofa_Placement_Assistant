...
function extractAnalysisSection(text, label) {
  const source = normalizeAnalysisText(text);
  if (!source) return "";

  const escapedLabel = label.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const pattern = new RegExp(
    `${escapedLabel}\\s*[：:]\\s*([\\s\\S]*?)(?=\\n\\s*(?:房间分析|沙发分析|摆放决策|生成约束|最终硬约束)\\s*[：:]|$)`,
    "i",
  );
  return (source.match(pattern)?.[1] || "").trim();
}

function splitSofaPlacementAnalysis(value) {
  const analysis = normalizeAnalysisText(value);
  return {
    roomAnalysis: extractAnalysisSection(analysis, "房间分析"),
    sofaAnalysis: extractAnalysisSection(analysis, "沙发分析"),
...
function buildSofaAnalysisRequest(payload) {
  ...
  parts.push({
    text: [
      ...
      `用户选择风格：${styleLine}`,
      "",
      "请严格按以下 4 个顶级区块输出，区块标题必须一字不差，后续生图会按标题拆分并分别注入：",
      "房间分析：",
      isCustomScene
        ? "1. 原房间保留清单：列出必须保持不变的门窗、墙地面、天花、已有家具/软装/装饰、采光方向、相机高度、焦距感、透视和画面氛围。严禁新增原图没有的窗户、阳台、门洞或大件家具。"
        : "1. 虚拟房间判断：根据沙发和风格判断最合理的室内空间功能、房间结构和氛围；房间必须有窗户、落地窗、阳台门或阳台等自然采光来源。",
      isCustomScene
        ? "2. 原房间空间线索判断：提取真实存在的窗户、落地窗、阳台门、阳台区域、阳光/自然光照射区、可用地面、通道、门柜开启区、已有家具关系、视觉中心和不应遮挡的位置。如果原图已有沙发、茶几、电视柜、窗户、地毯、绿植、边柜等，它们必须保持原样，不能被新增沙发替换或改造。"
        : "2. 虚拟房间空间线索：说明将创建哪些窗户/落地窗/阳台/阳台门、自然光方向、可用地面、通道、视觉中心和不应遮挡的位置；如果有 Reference Image 2，只提取风格和采光线索，不复制具体房间布局。",
      isCustomScene
        ? "3. 可用地面排除表：先列出至少 3 个候选区域，并逐一判断“有效/无效”。必须优先排除窄缝和拥挤夹角：原有沙发与窗帘/窗户之间、原有沙发与茶几/地毯之间、空气净化器/绿植/装饰物旁边、主通道、电视观看动线、柜门开启区。沙发需要完整落地、留出坐下和通行空间；不满足则判为无效。"
        : "3. 采光落位规划：说明窗边、落地窗边、阳台门边或阳台采光区的固定落位、自然光方向、地面承托和动线关系。",
      "沙发分析：",
      "1. 沙发类型与体量：例如单人沙发、双人沙发、转角沙发、躺椅/贵妃、功能沙发；说明视觉重量、高低比例、坐深、扶手/靠背体量。",
      "2. 必须保留的沙发视觉特征：颜色、材质、纹理、缝线、扶手、靠背、坐垫、脚架、脚踏、功能按钮、五金件、整体轮廓和比例。",
      "摆放决策：",
      "1. 不要套用固定候选位置。请给出 1 个主落位和 1 个备选落位，说明推荐落点、朝向、离墙/离窗/离阳台/离茶几/离通道关系、比例尺度和原因。主落位必须优先在窗边、落地窗边、阳台门边、阳台区域或自然光采光区；自定义房间下只能使用原图真实存在的采光结构，严禁为了靠窗而新增窗户/阳台。备选落位只作为用户后续手动改分析时的参考，不能用于远/中近/近景自动切换。",
      isCustomScene
        ? "2. 空间适配硬判断：主落位必须来自房间分析中判为“有效”的连续空白地面，并且尽量靠近原有窗边/阳台采光区。不要把沙发硬塞到原有主沙发旁、窗帘边、空气净化器旁或茶几与沙发之间的剩余缝隙。"
        : "2. 虚拟空间硬判断：主落位必须是虚拟房间中真实可用的窗边/阳台采光区，不得放到房间中央、通道、电视前方、柜门前方或远离采光面的普通墙边。",
      "3. 主落位锁定：用一句话写出后续所有景别必须复用的物理锚点，例如“锁定在某窗/阳台门/墙/地毯/地砖缝/茶几的左/右/前/后某片地面，朝向哪里，与哪些原有物体保持距离”。远景、中近景、近景只能改变镜头距离、视野和裁切，不能改变这个物理落位、房间侧位或朝向逻辑。",
      "4. 朝向与使用关系：必须明确沙发正面朝向、与电视/茶几/主沙发/窗户/阳台/房间中心的关系。沙发通常应朝向房间中心、茶几、电视或主座位区，角度自然约 15-45 度；不能背对主要活动区，不能正对窗玻璃贴放，不能像摆拍道具一样只面向镜头。",
      isCustomScene
        ? "5. 禁止落位：列出原图中不能摆放的位置，例如电视正前方、茶几上/茶几重叠区、主通道中央、窗户大面积遮挡区、原有沙发坐面/靠背上、柜门开启区、会造成过大比例的位置。"
        : null,
      "生成约束：",
      "1. 远/中近/近景延展：必须基于同一个主落位锁定，分别用【远景】、【中近景】、【近景】三行说明该如何拉开或靠近相机、扩大或收窄视野、保留多少窗边/阳台采光证据，而不是改变沙发所在位置、房间侧位、朝向逻辑或换用备选落位。后续生成时只会启用用户当前选择的那一个景别，另外两个景别必须被忽略。",
      includeModel
        ? "2. 模特摆放要求：用户选择添加模特，这是后续生成的硬性要求。请说明必须新增 1 位真实成人模特，且模特只能深坐在新增沙发上，臀部和大腿压在坐垫上，背部自然靠住靠背，脚放在地面或脚踏上，姿态生活化且受力真实；不能坐原有沙发、不能站在旁边、不能坐扶手或前沿、不能漂浮、不能遮挡沙发主体卖点，并要匹配房间光照、比例和透视。"
        : "2. 模特要求：用户未选择模特，不要新增人物、手、身体、倒影或人形轮廓。",
      "3. 透视与落地要求：相机高度、地面接触、阴影、反射、遮挡、避免穿模和动线阻挡。",
      "4. 最终硬约束复述：沙发必须固定在窗边、落地窗边、阳台门边或阳台采光区；远景、中近景、近景只能改变镜头距离和取景，不能改变沙发物理落位。",
...
function buildReferenceSofaPrompt(payload, hasStyleReferenceImage) {
  ...
  const { roomAnalysis, sofaAnalysis, fullAnalysis } = splitSofaPlacementAnalysis(payload.sofaAnalysis);
  ...
  return [
    ...
    "",
    `房间分析：${roomAnalysis || fullAnalysis || "未提供房间分析；请先自行根据参考图片判断房间/虚拟房间的采光、布局、动线和窗边/阳台采光区，再严格执行以上规则。"}`,
    "",
    `沙发分析：${sofaAnalysis || fullAnalysis || "未提供沙发分析；请先自行根据上传沙发图片判断外形、材质、颜色、扶手、靠背、坐垫、脚架和缝线细节，再严格执行以上规则。"}`,
    "",
